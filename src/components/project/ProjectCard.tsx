...
const ProjectCard: React.FC<ProjectCardProps> = ({ item }) => {
	return (
		<div
			key={item._id}
			className="group  w-full md:w-80 lg:w-72 h-80 py-6 px-3 dark:bg-dark bg-light hover:shadow-dark dark:hover:shadow-light shadow-2xl flex flex-col items-center rounded-t-xl"
		>
			...
			<div className="flex items-center justify-between space-x-16 mt-4">
				...
				<Link href={item.projectUrl} passHref>
					<a className="text-xl rounded-full px-3 py-2 text-white dark:text-dark font-bold bg-btnLight dark:bg-btnDark hover:bg-white hover:text-dark dark:hover:bg-white cursor-pointer transition-transform duration-300 ease-in">
						View Details
					</a>
				</Link>
			</div>
