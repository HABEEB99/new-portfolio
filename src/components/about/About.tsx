...
const About: React.FC<AboutProps> = ({ aboutData }) => {
	return (
		<section className="min-h-screen pt-[11vh] py-4" id="about">
			...
			<div className=" mt-6 md:mt-6 lg:mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-12 gap-x-[3.5rem] items-center justify-center">
				{aboutData.map((item) => (
					<div
						key={item._id}
						className="group  w-full md:w-[20rem] lg:w-[20rem] h-80 py-6 px-3 dark:bg-dark bg-light hover:shadow-dark dark:hover:shadow-light shadow-2xl flex flex-col items-center rounded-t-xl"
					>
